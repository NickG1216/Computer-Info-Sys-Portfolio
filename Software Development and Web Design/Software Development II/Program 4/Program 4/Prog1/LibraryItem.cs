...
// Makes use of IComparable to sort items in ascending order by title
...
namespace LibraryItems
{
    public abstract class LibraryItem : IComparable<LibraryItem>
    {
        ...
        // Precondition:  None
        // Postcondition: When this < item, method returns negative #
        //                When this == item, method returns zero
        //                When this > item, method returns positive #
        public int CompareTo(LibraryItem item)
        {
            //Handle nulls correctly (null < anything)

            if (item == null)// Only item is null?
                return 1;   // Any actual LibraryItem is greater than null

            if (this.Title.CompareTo(item.Title) != 0)
                return this.Title.CompareTo(item.Title);//Use Title to decide
            else if (this.Publisher.CompareTo(item.Publisher) != 0)
                return this.Publisher.CompareTo(item.Publisher);//Use Publisher to decide
            else if (this.CopyrightYear.CompareTo(item.CopyrightYear) != 0)
                return this.CopyrightYear.CompareTo(item.CopyrightYear);//Use Copyright Year to decide
            else if (this.CallNumber.CompareTo(item.CallNumber) != 0)
                return this.CallNumber.CompareTo(item.CallNumber);//Use Call Number
            else return this.LoanPeriod.CompareTo(item.LoanPeriod);//Use Loan Period
